...
from psi_convertion_supra import profiles_extraction
...
path_n100='/media/test-Samsung-SSD/roma/Work/simulations/lin_ITG_EP/supra/simplified_equil/noEP/toroidal_scan/low_n/n100_dt10_ntot7/orb5_res.h5'
...
path_n120='/media/test-Samsung-SSD/roma/Work/simulations/lin_ITG_EP/supra/simplified_equil/noEP/toroidal_scan/high_n/n120_dt10_ntot7/orb5_res.h5'
path_n90='/media/test-Samsung-SSD/roma/Work/simulations/lin_ITG_EP/supra/simplified_equil/noEP/toroidal_scan/low_n/n90_dt10_ntot7/orb5_res.h5'
path_n60='/media/test-Samsung-SSD/roma/Work/simulations/lin_ITG_EP/supra/simplified_equil/noEP/toroidal_scan/low_n/n60_dt10_ntot7/orb5_res.h5'
path=path_n100
...
ax1.set_xlabel(r'$s$', fontsize=18)
...
for ith in range(len(phi_theta)): 
    phi_averaged=phi_averaged+abs(phi_sc[t0,ith,:])
    #print(phi_averaged[100]-phi_sc[-1,ith,100])

wide_m=[-250,-240,-230,-220,-210,-200,-190, -180,-170]
narrow_m=np.linspace(-220,-200,21)
peak=0
for m_target in narrow_m:
    # find index in FFT ordering
    m_idx = np.where(m_modes == m_target)[0][0]

    # radial profile of this harmonic
    phi_radial = np.imag(phi_m[t0, m_idx, :])
    #print(np.where(phi_radial == max(abs(phi_radial)))[0][0])
    ax1.plot(phi_s, phi_radial, label=f"m={m_target}")
    peak=max(max(phi_radial),peak)
#ax1.plot(phi_s, phi_averaged/max(phi_averaged)*peak, label=f"ITG width")
#ax2=twinx(ax1)
#ax2.plot(s[128:232],-np.gradient(Te,s)[128:232]/np.min(np.gradient(Te,s)),label=r'$\nabla T$')
#ax2.set_ylabel(r"$\nabla T$", fontsize=16)
ax1.set_xlim(0.6,0.7)
#ax1.legend(fontsize=14)
#ax2.legend(fontsize=14, loc=4)
fig1.tight_layout()
#savefig('/media/test-Samsung-SSD/roma/Work/orb5_analysis/Pictures/linear_ITG/supra/phi_m_profiles_supra.pdf')
'''
...
max_pos26_TH10_maxw=max_phi_m(path_n60,26)
max_pos26_TH04_maxw=max_phi_m(path_n90,26)
max_pos26_TH100_maxw=max_phi_m(path_n100,26)

max_pos26_TH10_SDi=max_phi_m(path_n120,26)
